...
class Instance(object):
    def __init__(self, tokenizer, sentence_pack, post_vocab, deprel_vocab, postag_vocab, synpost_vocab, args):
        ...
        token_start = 1
        for i, w, in enumerate(self.tokens):    #检查分词的长度是否正确
        ...
        self.aspect_tags[0] = -1    # 这是给bert开头的[CLS]也变成了-1
        self.aspect_tags[self.length-1] = -1 #bert结尾的[SEP]也变成了-1
        ...
        for i in range(1, self.length-1):   # 上三角阵
        ...
        for i in range(1, self.length-1):   # 变成对称阵了
            for j in range(i, self.length-1):
