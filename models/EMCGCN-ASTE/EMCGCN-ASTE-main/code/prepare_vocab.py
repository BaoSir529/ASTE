...
def parse_args():
    ...
    parser.add_argument('--data_dir', default='../data/D2/lap14', help='data directory.')
...
def main():
    ...
    # lower tokens
    if args.lower:
        train_tokens, dev_tokens, test_tokens = [[t.lower() for t in tokens] for tokens in (train_tokens, dev_tokens, test_tokens)]
    ...
    post_counter = Counter(list(range(0, max_len))) # 存储相对位置信息
    syn_post_counter = Counter(list(range(0, 5)))   # 存储tree-based位置信息
...
def load_tokens(filename):
    with open(filename) as infile:
        ...
        for d in data:
            ...
            tokens.extend(sentence)     # 存储所有句子分词， 且允许重复，其长度为数据集总字符数
            deprel.extend(d['deprel'])
            postag_ca.extend(d['postag'])
            # postag.extend(d['postag'])
            n = len(d['postag'])
            tmp_pos = []
            for i in range(n):      # 存储任意两个单词之间的词性对，但是不知道干什么
                for j in range(n):
                    tup = tuple(sorted([d['postag'][i], d['postag'][j]]))
                    tmp_pos.append(tup)
            postag.extend(tmp_pos)      # 存储任意两个单词之间的词性对，而且允许重复
